...
if(isset($_POST['subAct'])){	
 ...
 case "input":	
	...
	try{
		...
		$name_users = $res3[0];
		$number = $res3[1];
		$message = 'Yth '. $name_users .' ,Silahkan Trf tepat Rp. '. rupiah($total_trf).' ke Nomor Rekening MANDIRI 165-006-015-1116  atau BCA 41-905-55-777 a/n PT. YellowFood Indonesia untuk konfirmasi pesanan otomatis';
		...
		// 	// }
		// }
		// echo"<script>alert('Input Success'); window.location = ('?page=cGVzYW5hbg&act=view&id=".Baggeo_Encrypt($orders)."')</script>";
		echo"<script>alert('Input Success'); window.location = ('?page=pending&act=view&id=".Baggeo_Encrypt($orders)."')</script>";	
		exit;
	  }catch(Exception $e){
